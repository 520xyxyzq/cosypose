...
def pose_residual_error(ests, scene_gt):
    """
    Simplified residual pose error computation script
    @param ests: [dict] Loaded bop results from .csv file
    @param scene_gt: [dict] Loaded object ground truth poses from .json file
    @return errs: [list of dicts] Calculated residual pose errors
    tx ty tz Rx Ry Rz
    """
    # Organize the estimates into recursive dicts
    ests_org = {}
        ...
        for obj_id, target in im_targets.items():
            ...
            for est_id, est in obj_ests_sorted:
                ...
                for gt_id, gt in enumerate(target):
                    ...
                    t_g = gt['cam_t_m2c']
                    # compute trans. and rot. error
                    pose_g = np.concatenate(
                        (R_g, t_g.reshape(3, 1)/1e3), axis=-1
                    )
                    pose_e = np.concatenate(
                        (R_e, t_e.reshape(3, 1)/1e3), axis=-1
                    )
                    pose_rel = gtsam.Pose3(pose_e).inverse() * \
                        gtsam.Pose3(pose_g)
                    e = gtsam.Pose3.Logmap(pose_rel).tolist()
